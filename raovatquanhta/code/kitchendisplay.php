...
if(isset($_POST['autoDisplayLocal']))
{
		...
		{
			//echo $getcurrentdate;
			if($getorder->get_kitchenPrepare($getcurrentdate)[0]['ordernumber'] != 0)
			{
				for($i = 0; $i < count($getorder->get_kitchenPrepare($getcurrentdate)); $i++)
				{
					echo "<tr class = 'orderTR'>
						<td class = 'eachOrderNumber' >order #" .$getorder->get_kitchenPrepare($getcurrentdate)[$i]['ordernumber']. "</td>
						<td class = 'eachOrderDescription'>" .$getorder->get_kitchenPrepare($getcurrentdate)[$i]['listofitems']. "</td>
						<td class = 'eachOrderPaid' >done</td>
						<td class = 'empordertime'>" .$getorder->get_kitchenPrepare($getcurrentdate)[$i]['employeenameID']. " " .$getorder->get_kitchenPrepare($getcurrentdate)[$i]['ordernumber']. " " .$getorder->get_kitchenPrepare($getcurrentdate)[$i]['time']. "</td>
						</tr>";
				}
			}
		}
