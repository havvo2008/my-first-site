<?php
// same as getorderDB.php
require_once('db.php');
class autoDisplayOrderListDB extends database
{
	private $link = null;
	...
	public function get_kitchenPrepare($todayDate)	//manager get all employee order
	{
		$sql = null; $statement = null; $result = null; $data = null;
		$sql = "SELECT * FROM ordertable where date = ? and kitchen = 'prepare';";
		$statement = $this->link->prepare($sql);
		$statement->bind_param("s", $todayDate );
		$statement->execute();
		$result = $statement->get_result();
